...
class LiveMerger:
    ...
    def __extract_host(self, url):
        ...
        if url in self.__host_cache:
            return self.__host_cache[url]
        ...
        self.__host_cache[url] = host
        return host

    def __count_host_channels(self):
        """统计每个主机的频道数量，使用缓存优化"""
        if self.__host_count is not None:
            return self.__host_count

        host_count = defaultdict(int)
        for _, _, url in self.__data:
            host = self.__extract_host(url)
            if host:
                host_count[host] += 1
    ...
    def find_top_hosts(self, n=3):
        ...
        if self.__top_hosts:
            return self.__top_hosts

        host_count = self.__count_host_channels()

        # 使用堆队列优化最大n个元素的查找
        import heapq
        self.__top_hosts = heapq.nlargest(n, host_count.items(), key=lambda x: x[1])
        return self.__top_hosts
    ...
    def __filter_channels(self):
        ...
        if not self.__top_hosts:
            self.find_top_hosts()

        top_host_set = {host for host, _ in self.__top_hosts}

        # 使用生成器表达式减少内存占用
        filtered_items = (
            (category, subgenre, url)
            for category, subgenre, url in self.__data
            if self.__extract_host(url) in top_host_set
        )

        for category, subgenre, url in filtered_items:
            self.__filtered_data[category].append((subgenre, url))

        return self.__filtered_data

    def format_output(self):
        """格式化输出结果，添加host统计信息"""
        if not self.__filtered_data:
            self.__filter_channels()
